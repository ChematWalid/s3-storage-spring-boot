...
import com.chematwalid.s3storage.image.ImageProcessingService;
import com.chematwalid.s3storage.image.ThumbnailatorImageProcessingService;
import com.chematwalid.s3storage.lifecycle.StorageObjectLifecycle;
import com.chematwalid.s3storage.s3.S3StorageService;
import com.chematwalid.s3storage.scan.ClamdInstreamScanner;
import com.chematwalid.s3storage.scan.VirusScanService;
import com.chematwalid.s3storage.scan.VirusScanner;
import com.chematwalid.s3storage.metrics.StorageMetrics;
import com.chematwalid.s3storage.stub.InMemoryStorageService;
import com.chematwalid.s3storage.validation.FileValidator;
...
public class StorageConfiguration {
    ...
    public VirusScanService virusScanService(StorageProperties properties, VirusScanner virusScanner,
                                             ObjectProvider<StorageMetrics> metricsProvider) {
        return new VirusScanService(properties, virusScanner,
                metricsProvider.getIfAvailable(StorageMetrics::noop));
    }

    @Bean
    @ConditionalOnMissingBean(StorageMetrics.class)
    public StorageMetrics storageMetrics() {
        return StorageMetrics.noop();
    }

    /**
     * Provides a UTC {@link Clock} when no other Clock bean is present.
     * Library consumers that already define their own Clock bean will have theirs used instead.
     */
    @Bean
    @ConditionalOnMissingBean
    ...
    /**
     * Registers the S3 storage health indicator when {@code storage.provider=s3}.
     */
    @Bean(name = "storageHealthIndicator")
    @ConditionalOnProperty(name = "storage.provider", havingValue = "s3")
    @ConditionalOnMissingBean(name = "storageHealthIndicator")
    public StorageHealthIndicator storageHealthIndicator(StorageProperties properties,
                                                         S3Client s3Client,
                                                         StorageMetrics metrics) {
        return new StorageHealthIndicator(properties, s3Client, metrics);
    }

    /**
     * Registers the ClamAV health indicator when virus scanning is enabled.
     */
    @Bean(name = "antivirusHealthIndicator")
    @ConditionalOnProperty(name = "storage.virus-scan.enabled", havingValue = "true")
    @ConditionalOnMissingBean(name = "antivirusHealthIndicator")
    public AntivirusHealthIndicator antivirusHealthIndicator(StorageProperties properties,
                                                              StorageMetrics metrics) {
        return new AntivirusHealthIndicator(properties, metrics);
    }

    /**
     * Registers the {@link StorageObjectLifecycle} helper that ties storage deletions to transaction commit.
     * Only created when spring-tx is on the classpath (optional dependency).
     */
    @Bean
    @ConditionalOnMissingBean(StorageObjectLifecycle.class)
    public StorageObjectLifecycle storageObjectLifecycle(StorageService storageService) {
        return new StorageObjectLifecycle(storageService);
    }

    /**
     * Registers the {@link com.chematwalid.s3storage.FileUrlResolver} for converting storage keys to URLs.
     */
    @Bean
    @ConditionalOnMissingBean(com.chematwalid.s3storage.FileUrlResolver.class)
    public com.chematwalid.s3storage.FileUrlResolver fileUrlResolver(StorageProperties properties) {
        return new com.chematwalid.s3storage.FileUrlResolver(properties);
    }
}
